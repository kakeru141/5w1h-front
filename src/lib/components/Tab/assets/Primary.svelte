<script lang="ts">
	...
	$: currentTabUndeBarWith = 0;
	$: if (tabsMenusElememt) {
		currentTabUndeBarWith = [...tabsMenusElememt.children][currentTabIndex].children[
			tabs[currentTabIndex].icon ? 1 : 0
		].clientWidth;
	}
</script>
...
<div class="tabs">
	<div class="tabs__under-bar__area">
		<span
			...
			style:width="{currentTabUndeBarWith}px"
			style:left="calc(((100% / {tabCount}) * {currentTabIndex}) + ((100% / {tabCount}) - {currentTabUndeBarWith}px)
			/ 2)"
		/>
